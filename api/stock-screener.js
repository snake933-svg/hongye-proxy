...
export default async function handler(req, res) {
  ...
  // ════════════════════════════════════════
  function toYahooFormat(records, id) {
    // 排序並只取最近 N 天
    ...
    return {
      ...
      indicators: {
        ...
        adjclose: [{
          adjclose: sorted.map(r => r.close), // TWSE 已是正確收盤，不需還原
        }],
      },
    };
  }
    ...
    if (source !== 'yahoo') {
      ...
      try {
        ...
        if (isOTC(symbol)) {
          ...
          records = await fetchTWSE(symbol, days);
          dataSource = 'TWSE';
        }

        if (records.length >= 5) {
          result = toYahooFormat(records, symbol);
        }
      } catch (e) {
